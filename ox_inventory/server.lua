...
lib.addCommand({'additem', 'giveitem'}, {
    ...
    params = {
        ...
        { name = 'degrade', type = 'number', help = 'Sets the "degrade" metadata to the value', optional = true },
    ...
    if item then
        ...
        -- Create a metadata table that includes degrade value if specified
        local metadata = {}
        if args.degrade then
            metadata.degrade = tonumber(args.degrade)
        end
        if args.type then
            metadata.type = tonumber(args.type) or args.type
        end

        -- Call the AddItem function with the new metadata
